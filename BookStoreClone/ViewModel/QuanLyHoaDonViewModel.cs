...
namespace BookStoreClone.ViewModel
{
    internal class QuanLyHoaDonViewModel : BaseViewModel
    {
        ...
        public ObservableCollection<int> ListChonSoLuong { get => _ListChonSoLuong; set { _ListChonSoLuong = value; OnPropertyChanged(); 
            }
        }
        ...
        public string DisCount  { get => _DisCount; set { _DisCount = value; OnPropertyChanged(); try
                {
                    foreach (CTHD hd in ListCTHD_BanSach)
                        if (hd.PhuongThuc == "Mua")
                            hd.ThanhTien = (int)hd.SoLuong * (int)hd.DonGiaBan * hd.TinhTrang / 100;
                    ...
                    TongGiaBan = (TongGiaBan * (100 - int.Parse(DisCount))) / 100; } catch { } } }
